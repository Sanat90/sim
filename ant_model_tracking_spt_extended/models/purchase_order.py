                ...
                for order_line in vals['order_line']:
                    ...
                    if len(order_line) >=3 and order_line[0] in [4,1] and order_line[2]:
                        ...
                        for col_name, col_info in tracked_fields.items():
                            if col_name in ['product_id','name','price_tax','price_total','price_subtotal','product_qty']:                            
                    ...
                    if len(order_line)>=3 and order_line[0] == 0 and order_line[2]:
                        line_id = order_line_obj.search([('product_id','=',order_line[2]['product_id']),('order_id','=',self.id),('price_unit','=',order_line[2]['price_unit']),('product_qty','=',order_line[2]['product_qty'])],limit=1)
                        for col_name, col_info in tracked_fields.items():
                            if col_name in ['product_id','name','price_tax','price_total','price_subtotal','product_qty']:
